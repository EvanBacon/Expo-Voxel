...
Game.prototype.addItem = function(item) {
  if (!item.tick) {
    var newItem = physical(item.mesh, this.potentialCollisionSet(), [
      item.size,
      item.size,
      item.size,
    ]);
...
Game.prototype.canCreateBlock = function(pos) {
  ...
  for (var i = 0, len = this.items.length; i < len; ++i) {
    ...
    var itemInTheWay =
      item.blocksCreation && item.aabb && bbox.intersects(item.aabb());
...
Game.prototype.loadPendingChunks = function(count) {
  ...
  for (var i = 0; i < count; i += 1) {
    ...
    var chunk = this.voxels.generateChunk(
      chunkPos[0] | 0,
      chunkPos[1] | 0,
      chunkPos[2] | 0,
    );
