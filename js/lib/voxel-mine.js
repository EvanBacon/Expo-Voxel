...
class Mine extends EventEmitter {
  ...
  enable() {
    this.reach.emitter.addListener('mining', this.onMining = (target) => {
      if (!target) {
    ...
    this.reach.emitter.addListener('start mining', this.onStartMining = (target) => {
      if (!target) {
        return;
      }
    ...
    this.reach.emitter.addListener('stop mining', this.onStopMining = (target) => {
      if (!target) {
        return;
      }
