(function() {
    ...
    module.exports.generateExamples = function() {
      return {
        Sphere: generate(
          [-16, -16, -16],
          [16, 16, 16],
          module.exports.generator['Sphere'],
        ),
        Noise: generate(
          [0, 0, 0],
          [16, 16, 16],
          module.exports.generator['Noise'],
        ),
        'Dense Noise': generate(
          [0, 0, 0],
          [16, 16, 16],
          module.exports.generator['Dense Noise'],
        ),
        Checker: generate(
          [0, 0, 0],
          [8, 8, 8],
        ...
        Hill: generate(
          [-16, 0, -16],
          [16, 16, 16],
          module.exports.generator['Hill'],
        ),
        Valley: generate(
          [0, 0, 0],
          [32, 32, 32],
          module.exports.generator['Valley'],
        ),
        'Hilly Terrain': generate(
          [0, 0, 0],
          [32, 32, 32],
          module.exports.generator['Hilly Terrain'],
        ),
      };
    };
  });

  require.define('/meshers/culled.js', function(
    require,
    module,
    exports,
    __dirname,
    ...
    function CulledMesh(volume, dims) {
      ...
      var vertices = [],
        ...
        n = -dims[0] * dims[1];
      for (
        B[2] = [false, true], x[2] = -1;
        x[2] < dims[2];
        B[2] = [true, ++x[2] < dims[2] - 1]
      )
        for (
          n -= dims[0], B[1] = [false, true], x[1] = -1;
          x[1] < dims[1];
          B[1] = [true, ++x[1] < dims[1] - 1]
        )
          for (
            n -= 1, B[0] = [false, true], x[0] = -1;
            x[0] < dims[0];
            B[0] = [true, ++x[0] < dims[0] - 1], ++n
          ) {
            //Read current voxel and 3 neighboring voxels using bounds check results
            var p = B[0][0] && B[1][0] && B[2][0] ? volume[n] : 0,
              b = [
                B[0][1] && B[1][0] && B[2][0] ? volume[n + 1] : 0,
                B[0][0] && B[1][1] && B[2][0] ? volume[n + dims[0]] : 0,
                B[0][0] && B[1][0] && B[2][1]
                  ? volume[n + dims[0] * dims[1]]
                  : 0,
              ];
            //Generate faces
            for (var d = 0; d < 3; ++d)
  ...
  require.define('/meshers/greedy.js', function(
    require,
    module,
    exports,
    __dirname,
  ...
  require.define('/meshers/monotone.js', function(
    require,
    module,
    exports,
    __dirname,
                ...
                if (n_side !== side) {
                  ...
                  while (bottom + 3 < top) {
                    ...
                    for (var j = 0; j < 2; ++j)
                      for (var k = 0; k < 2; ++k) {
                        delta[j][k] =
                          stack[top - 3 * (j + 1) + k + 1] - vert[k];
                      }
                    var det =
                      delta[0][0] * delta[1][1] - delta[1][0] * delta[0][1];
                    if (n_side === det > 0) {
                      break;
                    }
                    if (det !== 0) {
  ...
  require.define('/meshers/stupid.js', function(
    require,
    module,
    exports,
    __dirname,
  ...
  require.define('/test.js', function(
    require,
    module,
    exports,
    __dirname,
