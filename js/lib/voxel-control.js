...
class Control extends Stream {
  constructor(state, opts) {
    ...
    this.state = state;
    this._pitch_target = this._yaw_target = this._roll_target = this._target = null;
    ...
    this.state.x_rotation_accum = this.state.y_rotation_accum = this.state.z_rotation_accum = 0.0;
  ...
  tick = dt => {
    ...
    const { state, _target: target, jump_speed, speed } = this;
    // var okay_z = Math.abs(target.velocity.z) < this.max_speed;
    // var okay_x = Math.abs(target.velocity.x) < this.max_speed;
    const at_rest = target.atRestY();
    ...
    if (state.backward) {
      if (target.velocity.z < this.max_speed)
        target.velocity.z = Math.max(
          Math.min(
            this.max_speed,
            speed *
              dt *
              this.acceleration(this.z_accel_timer, this.accel_max_timer),
          ),
          target.velocity.z,
        );
    } else if (state.forward) {
      if (target.velocity.z > -this.max_speed)
        target.velocity.z = Math.min(
          Math.max(
            -this.max_speed,
            -speed *
              dt *
      ...
      if (target.velocity.x < this.max_speed)
        target.velocity.x = Math.max(
          Math.min(
            ...
            speed *
              dt *
              this.acceleration(this.x_accel_timer, this.accel_max_timer),
          ),
          target.velocity.x,
        );
    } else if (state.left) {
      ...
      if (!this.jumping && !at_rest) {
        ...
        if (this.jump_timer > 0) {
          target.velocity.y = Math.min(
            target.velocity.y + jump_speed * Math.min(dt, this.jump_timer),
            this.jump_max_speed,
          );
        }
    ...
    if (state.fire || state.firealt) {
      if (this.firing && this.needs_discrete_fire) {
        ...
        if (
          !this.fire_rate ||
          Math.floor(this.firing / this.fire_rate) !==
            Math.floor((this.firing + dt) / this.fire_rate)
        ) {
          this.onfire(state);
        }
        this.firing += dt;
    ...
    const x_rotation = this.state.x_rotation_accum * this.rotation_scale;
    const y_rotation = this.state.y_rotation_accum * this.rotation_scale;
    const z_rotation = this.state.z_rotation_accum * this.rotation_scale;
    const pitch_target = this._pitch_target;
    const yaw_target = this._yaw_target;
    const roll_target = this._roll_target;

    pitch_target.rotation.x = clamp(
      pitch_target.rotation.x + clamp(x_rotation, this.x_rotation_per_ms),
      this.x_rotation_clamp,
    );
    yaw_target.rotation.y = clamp(
      yaw_target.rotation.y + clamp(y_rotation, this.y_rotation_per_ms),
      this.y_rotation_clamp,
    );
    roll_target.rotation.z = clamp(
      roll_target.rotation.z + clamp(z_rotation, this.z_rotation_per_ms),
      this.z_rotation_clamp,
    );
  ...
  end = deltas => deltas && this.write(deltas);

  createWriteRotationStream = () => {
