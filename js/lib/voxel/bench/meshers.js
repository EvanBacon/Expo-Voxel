...
Object.keys(meshers).forEach(function(mesherName) {
  ...
  Object.keys(generators).forEach(function(generatorName) {
    ...
    for (var i = 0; i < iterations; i += 1)
      mesher(array, [chunkSize, chunkSize, chunkSize]);
...
Object.keys(times).forEach(function(mesher) {
  var meshTimes = times[mesher],
    ...
    meshTotal = meshKeys.reduce(function(n, k) {
      return meshTimes[k] + n;
    }, 0);

  console.log(
    '└─ ' + mesher + ': ' + limit(meshTotal / 1000 / iterations) + 'ms',
  );
  ...
  meshKeys
    ...
    .forEach(function(key) {
      console.log(
        '  └─ ' + key + ': ' + limit(meshTimes[key] / 1000 / iterations) + 'ms',
      );
    });
});
