...
export default class Physics {
  ...
  checkStack = (
    ...
    if (
      !this.world.isValidBlock(deltaX, deltaY, deltaZ) ||
      this.world.getBlock(deltaX, deltaY, deltaZ)
    ) {
      blockStackFull = true;
    } else if (
      !this.world.isValidBlock(deltaX, dYtorso, deltaZ) ||
      this.world.getBlock(deltaX, dYtorso, deltaZ)
    ) {
      blockStackFull = true;
    } else if (
      !this.world.isValidBlock(deltaX, dYhead, deltaZ) ||
      this.world.getBlock(deltaX, dYhead, deltaZ)
    ) {
      blockStackFull = true;
    }
    ...
    const xCollide: boolean =
      dest.x + radius > deltaX && dest.x - radius < deltaX + 1;
    const zCollide: boolean =
      dest.z + radius > deltaZ && dest.z - radius < deltaZ + 1;
    ...
    let xCollideEnd: number = circleXMax > deltaX + 1.0
      ? deltaX + 1.0
      : circleXMax;
    ...
    let zCollideEnd: number = circleZMax > deltaZ + 1.0
      ? deltaZ + 1.0
      : circleZMax;
    ...
    if (dest.x < deltaX) {
      ...
      if (dest.z < deltaZ) {
        ...
        let BLDepth: number =
          radius -
          Math.sqrt(
            (deltaX - dest.x) * (deltaX - dest.x) +
              (deltaZ - dest.z) * (deltaZ - dest.z)
          ); //bottom left
        if (circleZMax > deltaZ && circleXMax > deltaX && BLDepth > 0.0) {
          ...
          proj.normalize();
          dest.x = proj.x * BLDepth;
          dest.z = proj.z * BLDepth;
        }
        ...
        let TLDepth: number =
          ...
          Math.sqrt(
            (deltaX - dest.x) * (deltaX - dest.x) +
              (deltaZ + 1 - dest.z) * (deltaZ + 1 - dest.z)
          ); //top left
        if (circleZMin > deltaZ + 1.0 && circleXMax > deltaX && TLDepth > 0.0) {
          ...
          let proj = new THREE.Vector2(
            dest.x - deltaX,
            dest.z - (deltaZ + 1.0)
          );
          proj.normalize();
          dest.x = proj.x * TLDepth;
          dest.z = proj.z * TLDepth;
        }
      } else {
      ...
      if (dest.z < deltaZ) {
        ...
        let BRDepth: number =
          radius -
          Math.sqrt(
            (deltaX + 1.0 - dest.x) * (deltaX + 1.0 - dest.x) +
              (deltaZ - dest.z) * (deltaZ - dest.z)
          ); //bottom right
        if (circleZMax > deltaZ && circleXMin < deltaX + 1.0 && BRDepth > 0.0) {
          ...
          dest.x = proj.x * BRDepth;
          dest.z = proj.z * BRDepth;
        }
      } else if (dest.z > deltaZ + 1.0) {
        //top grid square
        let TRDepth: number =
          ...
          Math.sqrt(
            (deltaX + 1.0 - dest.x) * (deltaX + 1.0 - dest.x) +
              (deltaZ + 1.0 - dest.z) * (deltaZ + 1.0 - dest.z)
          ); //top right
        if (
          circleZMin < deltaZ + 1.0 &&
          circleXMin < deltaX + 1.0 &&
          TRDepth < 0.0
        ) {
          ...
          proj.normalize();
          dest.x = proj.x * TRDepth;
          dest.z = proj.z * TRDepth;
        }
      } else {
        //dest.y in cube's y-axis
  ...
  checkMovement2 = (pos, dir, height, radius) => {
    ...
    if (Math.abs(dir.y) > 0.0) {
      ...
      if (
        !this.world.isValidBlock(posX, dY, posZ) ||
        this.world.getBlock(posX, dY, posZ)
      ) {
        ...
        pos.x + radius > posX + 1 &&
        (!this.world.isValidBlock(posX + 1, dY, posZ) ||
          this.world.getBlock(posX + 1, dY, posZ))
      ) {
        ...
        pos.z + radius > posZ + 1 &&
        (!this.world.isValidBlock(posX, dY, posZ + 1) ||
          this.world.getBlock(posX, dY, posZ + 1))
      ) {
