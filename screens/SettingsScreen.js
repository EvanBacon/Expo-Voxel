...
export default class App extends React.Component {
	...
	async componentWillMount() {
		...
		const textureAsset = Expo.Asset.fromModule(
			...
			//
			// let fGeom = new THREE.PlaneGeometry( 10000, 10000 )
			// var floor = new THREE.Mesh( fGeom,
			// 	new THREE.MeshBasicMaterial( { color: 0x0000dd, opacity: 1, transparent: false } )
			// 	// new THREE.MeshLambertMaterial( { map: texture, vertexColors: THREE.VertexColors } )
			// );
			//
			// fGeom.rotateX( -(Math.PI / 2) );
			// // fGeom.translate( 0, 0, 0 );
			//
			// scene.add(floor)
			//
