...
class Reach extends EventEmitter {
  constructor(game, { reachDistance }) {
    super();
    this.game = game;
  ...
  startMining = () => this.emit('start mining', this.specifyTarget());

  stopMining = ev => {
    this.currentTarget = null;
    this.emit('stop mining', this.specifyTarget());
  };
  ...
  specifyTarget = () => {
    ...
    hit = this.game.raycastVoxels(
      this.game.cameraPosition(),
      this.game.cameraVector(),
      this.reachDistance,
    );
    ...
    sub = [
      fract(hit.position[0]),
      fract(hit.position[1]),
      fract(hit.position[2]),
    ];
...
module.exports = (game, opts) => new Reach(game, opts);
